            ...
            if (GameManager.singeton.gameStatus == GameStatus.PLAYING)  //is gamestatus is playing
            {
                switch (swipeType)
                {
                    case SwipeType.LEFT:                            //if we left swipe
                        endXPos = transform.position.x - 5;         //change endXPos by 3 to left
                        break;
                    case SwipeType.RIGHT:                           //if we right swipe
                        endXPos = transform.position.x + 5;         //change endXPos by 3 to right
                        break;
                }

                endXPos = Mathf.Clamp(endXPos, -5, 5);              //clamp endXPos between -3 and 3
